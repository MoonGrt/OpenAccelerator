...
class Conv2D(config: Conv2DConfig) extends Component {
  ...
  // --- ShiftColumn ---
  val col = new ShiftColumn(ShiftColumnConfig(dataWidth, kernelSize, rowNumDyn, rowNum))
  if (rowNumDyn) { col.io.rownum := io.rownum }
  col.io.pre <> io.pre
  // --- Convolution ---
...
class Conv2DLayerMultiChannel(layerCfg: Conv2DLayerConfig) extends Component {
  ...
  // --- StreamMap ---
  val streamMap = new StreamMap(StreamMapConfig(dataWidth, Seq.fill(channelNum)(kernelSize * kernelSize * convNum)))
  streamMap.io.kernelIn <> io.kernel
  // --- Conv2DLayer ---
